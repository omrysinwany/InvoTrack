...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = fbAuth.onIdTokenChanged(
      firebaseAuth,
      async (currentFirebaseUser) => {
        // Use aliased import
        console.log(
          "[AuthContext] onIdTokenChanged triggered. FirebaseUser UID:",
          currentFirebaseUser?.uid
        );
        setLoading(true);
        try {
          // We only process if there is a user. If null, it means logout.
          // The `processFirebaseUser` handles the null case gracefully.
          await processFirebaseUser(currentFirebaseUser, false, false, null);
        } catch (error) {
          console.warn(
            "[AuthContext] Error in onIdTokenChanged during processFirebaseUser, user state may be inconsistent:",
            error
          );
        } finally {
          setLoading(false);
          console.log(
            "[AuthContext] Finished processing onIdTokenChanged. Loading set to false."
          );
        }
      }
    );
    return () => {
      console.log("[AuthContext] Cleaning up onIdTokenChanged listener.");
      unsubscribe();
    };
