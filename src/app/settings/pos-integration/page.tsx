...
import type {
  PosConnectionConfig,
  SyncResult,
} from "@/services/pos-integration/pos-adapter.interface"; // Product type is not directly used here but by SyncResult
import { saveSyncedProductsAction } from "@/actions/pos-actions";
import {
  Loader2,
  Settings,
  Plug,
...
const systemConfigFields: Record<
  string,
  {
    key: string;
    labelKey: string;
...
export default function PosIntegrationSettingsPage() {
  ...
  const loadInitialData = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const result = await getAvailablePosSystemsAction();
      if (result.success) {
        setAvailableSystems(result.systems);
      } else {
        throw new Error(result.error || "Failed to load systems");
      }

      const settings = await getUserSettingsService(user.id);
      if (settings.posConfig) {
        setSelectedSystemId(settings.posConfig.systemId || "");
        setConfigValues(settings.posConfig.credentials || {});
      }
    } catch (error) {
    ...
    if (user) {
      getUserSettingsService(user.id)
        .then((savedSettings) => {
          if (
            savedSettings &&
            savedSettings.posConfig &&
            savedSettings.posConfig.systemId === systemId
          ) {
            setConfigValues(savedSettings.posConfig.credentials || {});
          } else {
            setConfigValues({});
          }
        })
        .catch((error) => {
          ...
          toast({
            title: t("error_title"),
            description: t("pos_toast_error_system_change"),
            variant: "destructive",
          });
        });
    }
  ...
  const handleTestConnection = async () => {
    ...
    console.log(
      `[POS Page] Testing connection for ${selectedSystemId} with config:`,
      fullConfig
    );
    try {
      const result = await testConnectionAction(fullConfig);
      setTestResult(result);
      toast({
        title: result.success
          ? t("pos_toast_test_success_title")
          : t("pos_toast_test_fail_title"),
        description: result.message,
        variant: result.success ? "default" : "destructive",
      });
    } catch (error: any) {
      console.error("[POS Page] Error during test connection call:", error);
      const errorMessage = t("pos_toast_test_error_desc", {
        message: error.message || t("pos_unknown_error"),
      });
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: t("pos_toast_save_success_desc", {
          systemName:
            availableSystems.find((s) => s.systemId === selectedSystemId)
              ?.systemName || "system",
        }),
      });
      setTestResult(null);
      ...
      toast({
        ...
        description: t("pos_toast_save_fail_desc", {
          message: error.message || t("pos_unknown_error"),
        }),
        variant: "destructive",
      });
  ...
  const handleSyncInventoryNow = async () => {
    ...
    setSyncResults([]);
    toast({
      title: t("pos_toast_sync_start_title"),
      description: t("pos_toast_sync_start_desc_inventory", {
        systemId: selectedSystemId,
      }),
    });
    ...
    try {
      ...
      const inventoryResult = await syncPosProductsAction(fullConfig);
      ...
      if (
        ...
        try {
          console.log(
            `[POS Page] Saving ${inventoryResult.products.length} synced products...`
          );
          // Pass products directly using the new data object structure
          await saveSyncedProductsAction(
            {
              products: inventoryResult.products,
              originalFileName: `POS_Inventory_Sync_${selectedSystemId}_${
                new Date().toISOString().split("T")[0]
              }`,
              docType: "deliveryNote", // A POS sync is treated like a delivery note
              existingDocumentId: null, // This is a new document
              invoiceNumber: `POS-SYNC-${Date.now()}`,
              supplierName: `POS Sync - ${selectedSystemId}`, // Use a generic supplier name for syncs
              totalAmount: null, // Can be calculated by the backend if needed
              paymentDueDate: null,
              invoiceDate: new Date(),
              paymentMethod: null,
              originalImagePreviewUri: null,
              compressedImageForFinalRecordUri: null,
              rawScanResultJson: null,
              paymentTerms: null,
              osekMorshe: null,
            },
            user.id
          ...
          setSyncResults((prev) => [
            ...
            {
              success: true,
              message: t("pos_toast_sync_save_products_desc", {
                count: inventoryResult.products?.length ?? 0,
              }),
            },
          ]);
          toast({
            title: t("pos_toast_sync_complete_title_inventory"),
            description: t("pos_toast_sync_complete_desc", {
              count: inventoryResult.products.length,
              systemId: selectedSystemId,
            }),
          });
        } catch (saveError: any) {
          ...
          setSyncResults((prev) => [
            ...
            {
              success: false,
              message: t("pos_toast_sync_save_fail_desc_products", {
          ...
          toast({
            ...
            description: t("pos_toast_sync_save_fail_desc_products_generic", {
              message: saveError.message || t("pos_unknown_error"),
            }),
            variant: "destructive",
          });
        }
        ...
        inventoryResult.success &&
        (!inventoryResult.products || inventoryResult.products.length === 0)
      ) {
        toast({
          title: t("pos_toast_sync_no_products_title"),
          description: t("pos_toast_sync_no_products_desc", {
            systemId: selectedSystemId,
          }),
        });
      } else {
        toast({
          ...
          description:
            ...
            t("pos_toast_sync_fail_desc_inventory_generic", {
              systemId: selectedSystemId,
            }),
          variant: "destructive",
        });
      ...
      console.error(
        `[POS Page] Error during ${selectedSystemId} inventory sync process:`,
        error
      );
      setSyncResults([
        {
          success: false,
          message: t("pos_toast_sync_error_desc_process", {
            message: error.message || t("pos_unknown_error"),
          }),
        },
      ]);
      toast({
        ...
        description: t("pos_toast_sync_error_desc_process_generic", {
          message: error.message || t("pos_unknown_error"),
        }),
        variant: "destructive",
      });
  ...
  const handleSyncSalesNow = async () => {
    ...
    toast({
      title: t("pos_toast_sync_start_title"),
      description: t("pos_toast_sync_start_desc_sales", {
        systemId: selectedSystemId,
      }),
    });
    ...
    try {
      ...
      const salesResult = await syncPosSalesAction(fullConfig);
      ...
      if (salesResult.success) {
        toast({
          ...
          description:
            salesResult.message ||
            t("pos_toast_sync_complete_desc_sales_generic", {
              systemId: selectedSystemId,
            }),
        });
      } else {
        toast({
          ...
          description:
            salesResult.message ||
            t("pos_toast_sync_fail_desc_sales_generic", {
              systemId: selectedSystemId,
            }),
          variant: "destructive",
        });
      ...
      console.error(
        `[POS Page] Error during ${selectedSystemId} sales sync process:`,
        error
      );
      setSyncResults([
        {
      ...
      <TooltipProvider>
        {fields.map((field) => (
          <div key={String(field.key)} className="space-y-2">
            <Label htmlFor={String(field.key)} className="flex items-center">
              {t(field.labelKey)}
              {field.tooltipKey && (
            ...
            <Input
              ...
              placeholder={t("pos_placeholder_enter_field", {
    ...
    <div className="container mx-auto p-4 sm:p-6 md:p-8 space-y-6">
      ...
      <Card className="shadow-md scale-fade-in">
        ...
        <CardContent className="space-y-6">
          ...
          {selectedSystemId && (
            <Card
              className="bg-muted/30 p-4 md:p-6 space-y-4 border scale-fade-in"
              style={{ animationDelay: "0.1s" }}
            >
              <h3 className="text-lg font-medium mb-4">
                {t("pos_configure_system_title", {
                  systemName:
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 pt-4">
                ...
                {testResult && (
                  <div
                    className={`flex items-center text-sm font-medium mt-2 sm:mt-0 sm:ml-4 ${
                      testResult.success
          ...
          {selectedSystemId && (
            <Card
              className="p-4 md:p-6 space-y-4 border scale-fade-in"
              style={{ animationDelay: "0.2s" }}
            >
              <h3 className="text-lg font-medium">
                {t("pos_manual_sync_title")}
              </h3>
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                    ...
                    >
                      {isSyncingInventory ? (
                        <>
                          <Loader2 className="mr-2 h-4 w-4 animate-spin" />{" "}
                          {t("pos_syncing_button")}...
                        </>
                      ) : (
                        <>
                          <RefreshCw className="mr-2 h-4 w-4" />{" "}
                          {t("pos_sync_inventory_now_button")}
                        </>
                      )}
                    </Button>
                  </div>
                  <p className="text-xs text-muted-foreground mt-2">
                    {t("pos_manual_sync_desc_inventory")}
                  </p>
                </div>
                <Separator />
                <div>
                  <h4 className="text-md font-medium mb-2">
                    {t("pos_sync_sales_title")}
                  </h4>
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                    <Button
                    ...
                    >
                      {isSyncingSales ? (
                        ...
                        <>
                          <RefreshCw className="mr-2 h-4 w-4" />{" "}
                          {t("pos_sync_sales_now_button")}
                        </>
                      )}
                    </Button>
                  </div>
                  <p className="text-xs text-muted-foreground mt-2">
                    {t("pos_manual_sync_desc_sales")}
                  </p>
                </div>
              </div>
                ...
                <div className="mt-4 space-y-2 text-sm">
                  ...
                  {syncResults.map((result, index) => (
                    ...
                    >
                      ...
                      <span>
                        {result.message}{" "}
                        {result.itemsSynced !== undefined
                          ? `(${result.itemsSynced} ${t(
                              "pos_items_synced_label"
                            )})`
                          : ""}
                      </span>
                    </div>
