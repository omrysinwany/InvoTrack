...
const getStorageKey = (baseKey: string, userId?: string): string => {
  if (!userId) {
    console.warn(
      `[getStorageKey OtherExpensesPage] Attempted to get storage key for base "${baseKey}" without a userId.`
    );
    return baseKey;
  }
...
export default function OtherExpensesPage() {
  ...
  const [specialExpenseAmounts, setSpecialExpenseAmounts] = useState<
    Record<string, number>
  >({});
  const [editingSpecialExpenseKey, setEditingSpecialExpenseKey] = useState<
    string | null
  >(null);
  const [tempSpecialExpenseAmount, setTempSpecialExpenseAmount] =
    useState<string>("");
  ...
  const [prefillDataForDialog, setPrefillDataForDialog] = useState<
    Partial<Omit<OtherExpense, "id"> & { isSpecialFixedExpense?: boolean }>
  >({});
  ...
  useEffect(() => {
    if (!authLoading && !user) {
      ...
      const categoriesStorageKey = getStorageKey(
        EXPENSE_CATEGORIES_STORAGE_KEY_BASE,
        user.id
      );
      const expensesStorageKey = getStorageKey(
        OTHER_EXPENSES_STORAGE_KEY_BASE,
        user.id
      );
      const templatesStorageKey = getStorageKey(
        EXPENSE_TEMPLATES_STORAGE_KEY_BASE,
        user.id
      );
      const specialAmountsStorageKey = getStorageKey(
        SPECIAL_EXPENSE_AMOUNTS_KEY_BASE,
        user.id
      );

      const storedCategories = localStorage.getItem(categoriesStorageKey);
      const defaultInternalCategories = [
        SPECIAL_CATEGORY_KEYS.ELECTRICITY,
        SPECIAL_CATEGORY_KEYS.WATER,
      ];
      ...
      if (storedCategories) {
        ...
        const uniqueParsed = parsedCategories.filter(
          (cat: string) =>
            !defaultInternalCategories.includes(cat.toLowerCase()) &&
            !Object.values(SPECIAL_CATEGORY_KEYS).includes(cat.toLowerCase())
        );
        finalCategories = [
          ...Object.values(SPECIAL_CATEGORY_KEYS),
          ...uniqueParsed,
        ];
      } else {
      ...
      if (!finalCategories.includes(SPECIAL_CATEGORY_KEYS.PROPERTY_TAX))
        finalCategories.push(SPECIAL_CATEGORY_KEYS.PROPERTY_TAX);
      if (!finalCategories.includes(SPECIAL_CATEGORY_KEYS.RENT))
        finalCategories.push(SPECIAL_CATEGORY_KEYS.RENT);

      setExpenseCategories(Array.from(new Set(finalCategories)));

      const generalTabCategories = finalCategories.filter(
        (catKey) =>
          catKey !== SPECIAL_CATEGORY_KEYS.PROPERTY_TAX &&
          catKey !== SPECIAL_CATEGORY_KEYS.RENT
      );

      if (
        generalTabCategories.length > 0 &&
        (!activeExpenseTab || !generalTabCategories.includes(activeExpenseTab))
      ) {
        ...
        setActiveExpenseTab(
          finalCategories.find(
            (cat) =>
              cat !== SPECIAL_CATEGORY_KEYS.PROPERTY_TAX &&
              cat !== SPECIAL_CATEGORY_KEYS.RENT
          ) || ""
        );
      }

      const storedExpenses = localStorage.getItem(expensesStorageKey);
      const parsedExpenses: OtherExpense[] = storedExpenses
      ...
      const storedSpecialAmounts = localStorage.getItem(
        specialAmountsStorageKey
      );
      const initialSpecialAmountsData: Record<string, number> =
        storedSpecialAmounts ? JSON.parse(storedSpecialAmounts) : {};
  ...
  const saveExpenseCategories = (categoriesToSave: string[]) => {
    if (typeof window !== "undefined" && user) {
      ...
      localStorage.setItem(
        ...
        JSON.stringify(
          Array.from(new Set(categoriesToSave.map((c) => c.toLowerCase())))
        )
      );
    }
  };
  ...
  const handleAddCategory = (newCategoryName: string) => {
    ...
    if (!trimmedName) {
      toast({
        title: t("error_title"),
        description: t("accounts_toast_category_name_empty_desc"),
        variant: "destructive",
      });
      return;
    }
    ...
    const categoryExists = expenseCategories.some(
      (catKey) =>
        catKey === internalKey ||
        t(`accounts_other_expenses_tab_${catKey}` as any, {
          defaultValue: catKey,
        }).toLowerCase() === trimmedName.toLowerCase()
    );
    if (categoryExists) {
      toast({
        title: t("accounts_toast_category_exists_title"),
        description: t("accounts_toast_category_exists_desc", {
    ...
    toast({
      title: t("accounts_toast_category_added_title"),
      description: t("accounts_toast_category_added_desc", {
        categoryName: trimmedName,
      }),
    });
  ...
  const handleAddOrUpdateExpense = (
    expenseData: Omit<OtherExpense, "id" | "_internalCategoryKey"> & {
      id?: string;
      _internalCategoryKey?: string;
    },
    ...
    if (!expenseData.description.trim()) {
      toast({
        title: t("error_title"),
        description: t("accounts_toast_expense_desc_empty_desc"),
        variant: "destructive",
      });
      return;
    }
    if (expenseData.amount <= 0) {
      toast({
        title: t("accounts_toast_expense_invalid_amount_title"),
        description: t("accounts_toast_expense_invalid_amount_desc"),
        variant: "destructive",
      });
      return;
    }
    if (!expenseData.category) {
      toast({
        title: t("error_title"),
        description: t("accounts_toast_expense_category_empty_desc"),
        variant: "destructive",
      });
      return;
    }
    if (!expenseData.date || !isValid(parseISO(expenseData.date))) {
      toast({
        title: t("error_title"),
        description: t("accounts_toast_expense_invalid_date_desc"),
        variant: "destructive",
      });
      return;
    }
    ...
    const internalCatKey =
      expenseData._internalCategoryKey ||
      expenseCategories.find(
        (catKey) =>
          t(`accounts_other_expenses_tab_${catKey}` as any, {
            defaultValue: catKey,
          }).toLowerCase() === expenseData.category.toLowerCase()
      ) ||
      expenseData.category.toLowerCase().replace(/\s+/g, "_");

    const categoryLabel = t(
      `accounts_other_expenses_tab_${internalCatKey}` as any,
      {
        defaultValue:
          internalCatKey.charAt(0).toUpperCase() + internalCatKey.slice(1),
      }
    );
    ...
    if (expenseData.id) {
      updatedExpenses = otherExpenses.map((exp) =>
        exp.id === expenseData.id
          ? ({ ...exp, ...expenseToSave } as OtherExpense)
          : exp
      );
    ...
    toast({
      title: toastMessage,
      description: t("accounts_toast_expense_added_desc", {
    ...
    if (templateDetails?.saveAsTemplate) {
      const newTemplate: ExpenseTemplate = {
        ...
        name:
          ...
          `${expenseToSave.description.substring(0, 20)} Template`,
      ...
      toast({
        title: t("accounts_toast_template_saved_title"),
        description: t("accounts_toast_template_saved_desc", {
  ...
  const handleDeleteExpense = (expenseId: string) => {
    ...
    saveOtherExpenses(updatedExpenses);
    toast({
      title: t("accounts_toast_expense_deleted_title"),
      description: t("accounts_toast_expense_deleted_desc"),
    });
  };
  ...
  const handleSaveSpecialExpense = (key: string) => {
    ...
    const categoryLabel = t(`accounts_other_expenses_tab_${key}` as any, {
      defaultValue: key.charAt(0).toUpperCase() + key.slice(1),
    });
    const currentMonthYear = format(new Date(), "MMMM yyyy");
    const description = `${categoryLabel} - ${currentMonthYear}`;

    const existingSpecialExpenseForThisMonth = otherExpenses.find(
      (exp) =>
    ...
    toast({
      ...
      description: `${categoryLabel} ${t(
        "accounts_toast_amount_updated_desc"
      )}`,
    });
  };
  ...
  const getCategoryIcon = (categoryKey: string): React.ElementType => {
    ...
    if (lowerCategoryKey.includes(SPECIAL_CATEGORY_KEYS.ELECTRICITY))
      return Zap;
    if (lowerCategoryKey.includes(SPECIAL_CATEGORY_KEYS.WATER)) return Droplet;
    if (lowerCategoryKey.includes(SPECIAL_CATEGORY_KEYS.PROPERTY_TAX))
      return Home;
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        {[SPECIAL_CATEGORY_KEYS.PROPERTY_TAX, SPECIAL_CATEGORY_KEYS.RENT].map(
          (specialCatKey) => {
            let categoryLabel = "";
            ...
            return (
              <Card
                key={specialCatKey}
                className="shadow-md scale-fade-in delay-100 flex flex-col"
              >
                <CardHeader className="pb-3 flex flex-row items-start justify-between">
                  <div>
                    ...
                    <CardDescription>
                      {t("accounts_other_expenses_recurring_desc", {
                ...
                <CardContent className="space-y-1 flex-grow">
                  {editingSpecialExpenseKey === specialCatKey ? (
                    <div className="space-y-2">
                      <Label
                        htmlFor={`special-amount-${specialCatKey}`}
                        className="sr-only"
                      >
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setTempSpecialExpenseAmount(e.target.value)
                        }
                        placeholder={t(
                          "accounts_fixed_expense_amount_placeholder",
                  ...
                  {!latestExpenseForCategory &&
                    editingSpecialExpenseKey !== specialCatKey && (
                      <p className="text-sm text-muted-foreground py-1">
                        {t("accounts_other_expenses_no_record_yet")}
                      </p>
                    )}
                </CardContent>
                <CardFooter className="border-t pt-3 pb-3">
                  {editingSpecialExpenseKey === specialCatKey ? (
                    <div className="flex gap-2 w-full">
                      <Button
                        onClick={handleCancelEditSpecialExpense}
                        variant="outline"
                        size="sm"
                        className="flex-1"
                      >
                      ...
                      <Button
                        onClick={() => handleSaveSpecialExpense(specialCatKey)}
                        size="sm"
      ...
      <Card className="shadow-md scale-fade-in delay-200">
        <CardHeader>
          <CardTitle className="text-xl sm:text-2xl font-semibold text-primary flex items-center">
            ...
            {t("accounts_other_expenses_variable_title")}
          </CardTitle>
          <CardDescription>
            {t("accounts_other_expenses_variable_desc")}
          </CardDescription>
        </CardHeader>
          ...
          >
            <div className="flex items-center gap-2 border-b pb-2">
              <ScrollArea className="w-full whitespace-nowrap">
                <TabsList className="inline-flex h-10 items-center justify-start rounded-lg bg-muted p-1 text-muted-foreground">
                  {tabCategories.map((categoryKey) => (
                    ...
                    >
                      {t(
                        `accounts_other_expenses_tab_${categoryKey
                          .toLowerCase()
              ...
              <Button
                ...
                className="ml-auto flex-shrink-0 text-primary hover:bg-primary/10"
              >
                <PlusCircle className="h-5 w-5" />
                <span className="sr-only">
                  {t("accounts_add_category_button")}
                </span>
              </Button>
            </div>
            {tabCategories.map((categoryKey) => (
              <TabsContent
                key={categoryKey}
                value={categoryKey}
                className="mt-4 min-h-[200px] tabs-content-fade-in"
              >
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {otherExpenses.filter(
                    (exp) => exp._internalCategoryKey === categoryKey
                  ).length > 0 ? (
                    otherExpenses
                      .filter((exp) => exp._internalCategoryKey === categoryKey)
                      .sort(
                        (a, b) =>
                      ...
                      .map((expense) => (
                        <Card
                          key={expense.id}
                          className="bg-card shadow-sm hover:shadow-md transition-shadow duration-200 flex flex-col"
                        >
                    ...
                    <div className="sm:col-span-2 lg:col-span-3 text-center py-8 text-muted-foreground">
                      {React.createElement(getCategoryIcon(categoryKey), {
                        className: "mx-auto h-12 w-12 mb-2 opacity-50",
                      })}
                      <p className="text-sm ">
                        {t("accounts_other_expenses_no_expenses_in_category")}
                      </p>
                      <Button
                        variant="link"
                        size="sm"
                        onClick={() =>
                          openAddDialogForVariableCategory(categoryKey)
                        }
                        className="mt-1 text-primary"
                      >
                        {t("accounts_add_expense_button")}{" "}
                        {t(
                          `accounts_other_expenses_tab_${categoryKey
                            .toLowerCase()
                            .replace(/\s+/g, "_")}` as any,
                          { defaultValue: categoryKey.toLowerCase() }
                        )}
                      </Button>
                    </div>
                  )}
                ...
                <div className="flex justify-end pt-4 mt-4 border-t">
                  <Button
                    onClick={() =>
                      openAddDialogForVariableCategory(activeExpenseTab)
                    }
                    className="bg-primary hover:bg-primary/90"
                  >
                    <PlusCircle className="mr-2 h-4 w-4" />{" "}
                    {t("accounts_add_expense_button_general")}
                  </Button>
                </div>
              </TabsContent>
            ))}
      ...
      <AddExpenseDialog
        ...
        preselectedCategory={
          prefillDataForDialog._internalCategoryKey ||
          editingExpense?._internalCategoryKey ||
          activeExpenseTab
        }
