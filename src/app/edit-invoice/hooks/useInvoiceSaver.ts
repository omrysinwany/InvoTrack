...
interface UseInvoiceSaverProps {
  ...
  taxDetailsToSave: Partial<Invoice>;
  ...
  onSaveSuccess: (savedInvoice: BackendInvoice) => void;
...
export function useInvoiceSaver({
  ...
  const proceedWithFinalSave = useCallback(
    async (finalProductsToSave: EditableProduct[]) => {
      ...
      try {
        ...
        const finalFileNameForSave = initialOriginalFileName;
        ...
        if (taxDetailsToSave.date instanceof Timestamp) {
          finalInvoiceDateForSave = taxDetailsToSave.date;
        } else if (typeof taxDetailsToSave.date === "string") {
          const parsed = parseISO(taxDetailsToSave.date);
          if (isValid(parsed))
            finalInvoiceDateForSave = Timestamp.fromDate(parsed);
          else finalInvoiceDateForSave = taxDetailsToSave.date;
        } else if (
          taxDetailsToSave.date instanceof Date &&
          isValid(taxDetailsToSave.date)
        ) {
          finalInvoiceDateForSave = Timestamp.fromDate(taxDetailsToSave.date);
        }
        ...
        // Create a sanitized object to pass to the service
        const finalInvoiceData = {
          products: productsForService,
          originalFileName: initialOriginalFileName || "Unnamed Document",
          docType: docType as
            | "deliveryNote"
            | "invoice"
            | "invoiceReceipt"
            | "receipt",
          userId: user.id,
          existingDocumentId: initialTempInvoiceId || initialInvoiceIdParam,
          invoiceNumber: finalInvoiceNumberForSave || null,
          supplierName: finalSupplierNameForSave || null,
          totalAmount: finalTotalAmountForSave ?? null,
          paymentDueDate: paymentDueDateForDoc || null,
          invoiceDate: finalInvoiceDateForSave || null,
          paymentMethod: taxDetailsToSave.paymentMethod || null,
          originalImagePreviewUri: displayedOriginalImageUrl || null,
          compressedImageForFinalRecordUri: displayedCompressedImageUrl || null,
          rawScanResultJson: rawScanResultJsonForSave || null,
          paymentTerms: paymentTermStringForDocument || null,
          osekMorshe: taxDetailsToSave.osekMorshe || null,
        };

        const { finalInvoiceRecord, savedOrUpdatedProducts } =
          await finalizeSaveProductsService(finalInvoiceData);

        // --- Sync with POS in the background ---
        if (savedOrUpdatedProducts.length > 0) {
          syncProductsWithPosService(savedOrUpdatedProducts, user.id).catch(
            (e) => {
              console.error("Background POS sync failed:", e);
  ...
  const handleMarkAsPaid = useCallback(async () => {
    ...
    try {
      ...
      const updatedDocPartial: Partial<BackendInvoice> = {
      ...
      onSaveSuccess(updatedDocPartial as BackendInvoice);
    } catch (error: any) {
