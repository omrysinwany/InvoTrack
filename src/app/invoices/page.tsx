...
import type { Invoice, Supplier } from "@/services/types";
  ...
  const filteredAndSortedInvoices = useMemo(() => {
    ...
    if (dateRange?.from) {
      ...
      result = result.filter((inv) => {
        ...
        const invDate = (inv.uploadTime as Timestamp).toDate();
    ...
    if (filterPaymentStatus) {
      result = result.filter((inv) => {
        ...
        if (inv.paymentStatus === "pending_payment" && inv.dueDate) {
          const dueDateObj = (inv.dueDate as Timestamp).toDate();
          if (
            isValid(dueDateObj) &&
            isBefore(dueDateObj, new Date()) &&
            !isSameDay(dueDateObj, new Date())
                ...
                <div className="space-y-2">
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      ...
                      >
                        <CalendarDays className="mr-2 h-4 w-4" />
                        {editedInvoiceData.dueDate ? (
                          formatDateForDisplay(
                            editedInvoiceData.dueDate as Timestamp,
                            "PP"
                          )
                        ) : (
                    ...
                    <PopoverContent className="w-auto p-0">
                      <Calendar
                        mode="single"
                        selected={
                          editedInvoiceData.dueDate instanceof Timestamp
                            ? editedInvoiceData.dueDate.toDate()
                            : (editedInvoiceData.dueDate as Date | undefined)
                        }
                        onSelect={(date) =>
                          handleEditDetailsInputChange("dueDate", date)
                        }
                        initialFocus
                      />
                    </PopoverContent>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-sm">
                  ...
                  <div>
                    ...
                    <p>
                      ...
                      {formatDateForDisplay(
                        selectedInvoiceDetails?.dueDate as Timestamp,
                        "PP"
                      )}
                    </p>
                    <p>
