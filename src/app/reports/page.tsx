...
const chartConfig = {
  value: {
    labelKey: "reports_chart_label_value",
    color: "hsl(var(--chart-1))",
  },
  count: {
    labelKey: "reports_chart_label_count",
    color: "hsl(var(--chart-2))",
  },
  sales: {
    labelKey: "reports_chart_label_sales",
    color: "hsl(var(--chart-3))",
  },
  quantitySold: {
    labelKey: "reports_chart_label_qty_sold",
    color: "hsl(var(--chart-4))",
  },
  documents: {
    labelKey: "reports_chart_label_documents",
    color: "hsl(var(--chart-5))",
  },
  expenses: {
    labelKey: "reports_chart_label_expenses",
    color: "hsl(var(--chart-1))",
  },
  profit: {
    labelKey: "reports_profitability_profit",
    color: "hsl(var(--chart-2))",
  },
...
export default function ReportsPage() {
  ...
  const [expensesByCategoryData, setExpensesByCategoryData] = useState<
    ExpenseByCategory[]
  >([]);
  const [supplierLiabilitiesData, setSupplierLiabilitiesData] = useState<
    SupplierLiability[]
  >([]);
  ...
  const [productProfitabilityData, setProductProfitabilityData] = useState<
  ...
  const [drillDownDialogData, setDrillDownDialogData] = useState<
    OtherExpense[]
  >([]);
    ...
    const generateReports = () => {
      ...
      const filteredInvoicesByDate = invoices.filter((invoice) => {
        ...
        if (invoice.uploadTime instanceof Timestamp)
          invoiceDate = invoice.uploadTime.toDate();
        else if (
          typeof invoice.uploadTime === "string" &&
          isValid(parseISO(invoice.uploadTime))
        )
          invoiceDate = parseISO(invoice.uploadTime);
      ...
      const totalPotentialGrossProfit =
        calculateTotalPotentialGrossProfit(inventory);
      const averageInvoiceValue = calculateAverageOrderValue(
        filteredInvoicesByDate.filter((inv) => inv.status === "completed")
      );
      const activeSuppliers = new Set(
        filteredInvoicesByDate.map((inv) => inv.supplierName).filter(Boolean)
      );

      const totalInvoiceCostsPeriod = filteredInvoicesByDate.reduce(
        (sum, inv) => sum + (inv.totalAmount || 0),
        0
      );
      const totalOtherExpensesPeriod = filteredOtherExpensesByDate.reduce(
        (sum, exp) => sum + exp.amount,
        0
      );
      const totalRecordedExpensesPeriod =
        totalInvoiceCostsPeriod + totalOtherExpensesPeriod;

      const openInvoicesPeriod = filteredInvoicesByDate.filter(
        (inv) =>
          inv.paymentStatus === "unpaid" ||
          inv.paymentStatus === "pending_payment"
      );
      const numberOfOpenInvoices = openInvoicesPeriod.length;
      const totalAmountDueFromOpenInvoices = openInvoicesPeriod.reduce(
        (sum, inv) => sum + (inv.totalAmount || 0),
        0
      );
      ...
      const votData = [];
      const currentDateForVOT = dateRange?.from
        ? new Date(dateRange.from)
        : subMonths(new Date(), 6);
      const endDateForVOT = dateRange?.to || new Date();
      const numPointsVOT = isMobile ? 5 : 10;
      const stepDurationVOT =
        (endDateForVOT.getTime() - currentDateForVOT.getTime()) /
        Math.max(1, numPointsVOT - 1);

      for (let i = 0; i < numPointsVOT; i++) {
        const pointDate = new Date(
          currentDateForVOT.getTime() + i * stepDurationVOT
        );
        ...
        const invoicesUpToPointDate = invoices.filter((inv) => {
          ...
          return (
            invDate &&
            invDate <= pointDate &&
            inv.documentType === "deliveryNote"
          );
        });
        const simulatedValue =
          invoicesUpToPointDate.reduce(
            (sum, inv) => sum + (inv.totalAmount || 0),
            0
          ) *
          (0.8 + Math.random() * 0.4);
      ...
      const catDistData = categories
        .map((catKey) => {
          ...
          const categoryLabel = t(
            `accounts_other_expenses_tab_${catKey}` as any,
            {
              defaultValue:
      ...
      const procVolStepDuration =
        (endDateForProcVol.getTime() - currentDateForProcVol.getTime()) /
        Math.max(1, procVolNumPoints - 1);

      for (let i = 0; i < procVolNumPoints; i++) {
        const pointDate = new Date(
          currentDateForProcVol.getTime() + i * procVolStepDuration
        );
      ...
      const topProducts = inventory
        .filter(
          (p) =>
            p.salePrice !== undefined && p.salePrice !== null && p.salePrice > 0
        )
        .map((p) => ({
          id: p.id,
          name:
            ...
            (p.description
              ...
              : ""),
          quantitySold:
            Math.floor(Math.random() * (p.quantity > 0 ? p.quantity / 2 : 5)) +
            1, // Mock sales data
          totalValue:
            (p.salePrice || 0) *
            (Math.floor(Math.random() * (p.quantity > 0 ? p.quantity / 2 : 5)) +
      ...
      const alerts: StockAlert[] = inventory.reduce((acc, p) => {
        ...
        if (currentQuantity === 0) {
          ...
          acc.push({
            ...
            minStock: p.minStockLevel,
            maxStock: p.maxStockLevel,
            isDefaultMinStock: isDefaultMin,
          });
        }
        return acc;
      }, [] as StockAlert[]);
      ...
      const pnlOperatingExpenses = filteredOtherExpensesByDate.reduce(
        (sum, exp) => sum + exp.amount,
        0
      );
      ...
      filteredInvoicesByDate
        ...
        .forEach((inv) => {
          if (inv.supplierName && typeof inv.supplierName === "string") {
            const current = liabilitiesMap.get(inv.supplierName) || {
              totalDue: 0,
              invoiceCount: 0,
            };
      ...
      setSupplierLiabilitiesData(
        Array.from(liabilitiesMap.entries())
      ...
      const profitability = inventory
        ...
        .filter(
          (p) =>
            p.potentialProfitPerUnit !== undefined &&
            p.potentialProfitPerUnit >= 0
        );
  ...
  const handleDatePreset = (
    preset: "7d" | "30d" | "currentMonth" | "currentQuarter"
  ) => {
  ...
  const exportToCsv = (
    ...
    if (data.length === 0) {
      toast({
        title: t("reports_toast_no_data_to_export_title"),
        description: t("reports_toast_no_data_to_export_desc"),
      });
      return;
    }
    ...
    const rows = data.map((item) =>
      headers
        .map((h) => {
          if (
            h.key === "potentialProfitPerUnit" ||
            h.key === "profitMarginPercent" ||
            h.key === "unitPrice" ||
            h.key === "salePrice"
          ) {
            return item[h.key] !== undefined
              ? formatNumber(item[h.key], t, {
                  decimals: h.key === "profitMarginPercent" ? 1 : 0,
                  currency: h.key !== "profitMarginPercent",
                })
              : "-";
          }
          if (h.key === "totalDue" || h.key === "totalAmount") {
            return formatNumber(item[h.key], t, {
              currency: true,
              decimals: 0,
            });
          }
    ...
    toast({
      title: t("reports_toast_export_success_title"),
      description: t("reports_toast_export_success_desc", {
  ...
  const handlePieChartClick = (data: any) => {
    if (data && data.activePayload && data.activePayload.length > 0) {
      ...
      const categoryData = expensesByCategoryData.find(
        (cat) => cat.category === clickedCategoryName
      );
      if (categoryData) {
        setDrillDownDialogTitle(
          t("reports_drilldown_title_expenses_for_category", {
  ...
  if (authLoading || (isLoading && !kpis) || !user) {
    return (
      <div className="container mx-auto p-4 md:p-8 flex justify-center items-center min-h-[calc(100vh-var(--header-height,4rem))] bg-transparent">
  ...
  return (
    <TooltipProvider>
      <div className="container mx-auto p-2 sm:p-4 md:p-6 space-y-4 bg-transparent">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {(["7d", "30d", "currentMonth", "currentQuarter"] as const).map(
            (preset) => (
              <Button
                key={preset}
                variant="outline"
        ...
        <div className="grid gap-2 sm:gap-3 grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
          {[
            {
              titleKey: "reports_kpi_total_value",
            ...
            {
              titleKey: "reports_kpi_total_items",
              value: kpis?.totalItems,
              Icon: Package,
              decimals: 0,
            },
            {
              titleKey: "reports_kpi_low_stock_items",
              value: kpis?.lowStockItems,
              Icon: AlertTriangle,
              decimals: 0,
            },
            {
              titleKey: "reports_kpi_potential_gross_profit_short",
              value: kpis?.totalPotentialGrossProfit,
              Icon: HandCoins,
              currency: true,
              decimals: 0,
            },
            {
              titleKey: "reports_kpi_avg_invoice_value_short",
              value: kpis?.averageInvoiceValue,
              Icon: FileTextIcon,
              currency: true,
              decimals: 0,
            },
            {
              titleKey: "reports_kpi_active_suppliers_short",
              value: kpis?.totalActiveSuppliers,
              Icon: Briefcase,
              decimals: 0,
            },
            {
              titleKey: "reports_kpi_total_invoice_costs_period",
              value: kpis?.totalInvoiceCostsPeriod,
              Icon: Banknote,
              currency: true,
              decimals: 0,
            },
            {
              titleKey: "reports_kpi_total_other_expenses_period",
              value: kpis?.totalOtherExpensesPeriod,
              Icon: ListChecks,
              currency: true,
              decimals: 0,
            },
            {
              titleKey: "reports_kpi_total_recorded_expenses_period",
              value: kpis?.totalRecordedExpensesPeriod,
              Icon: DollarSign,
              currency: true,
              decimals: 0,
            ...
            {
              titleKey: "reports_kpi_open_invoices_count_period",
              value: kpis?.numberOfOpenInvoices,
              Icon: FileWarning,
              decimals: 0,
            },
            {
              titleKey: "reports_kpi_open_invoices_amount_period",
              value: kpis?.totalAmountDueFromOpenInvoices,
              Icon: Banknote,
              currency: true,
              decimals: 0,
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-5 w-5 -mt-1 -mr-1 opacity-70 hover:opacity-100"
                    >
                      <kpi.Icon
                        className={cn(
                  ...
                  <TooltipContent side="top" align="end" className="max-w-xs">
                    <p className="text-xs">
                      {t(`${kpi.titleKey}_desc` as any, {
                        defaultValue: t("reports_kpi_default_desc"),
                      })}
                    </p>
                  </TooltipContent>
                </Tooltip>
          ...
          <CardHeader className="pb-2 sm:pb-4 flex flex-row items-center justify-between">
            <div className="flex items-center">
              ...
              <Tooltip>
                ...
                <TooltipContent>
                  <p>{t("reports_profitability_tooltip")}</p>
                </TooltipContent>
              </Tooltip>
            </div>
            <Button
              ...
              onClick={() =>
                exportToCsv(
                  ...
                  [
                    ...
                    {
                      key: "unitPrice",
                      label: t("reports_col_unit_price_cost"),
                    },
                    { key: "salePrice", label: t("reports_col_sale_price") },
                    {
                      key: "potentialProfitPerUnit",
                      label: t("reports_col_profit_per_unit"),
                    },
                    {
                      key: "profitMarginPercent",
                      label: t("reports_col_profit_margin_percent"),
                    },
                  ],
                  "product_profitability_report"
                )
              }
              disabled={productProfitabilityData.length === 0}
              className="text-xs"
            >
              <Download className="mr-1 h-3 w-3" />{" "}
              {t("reports_export_csv_button")}
            </Button>
          </CardHeader>
            ...
            {productProfitabilityData.length > 0 ? (
              <div className="overflow-x-auto">
                <Table className="min-w-full">
                  <TableHeader>
                    <TableRow>
                      <TableHead className="text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                        {t("reports_col_product")}
                      </TableHead>
                      <TableHead className="text-[10px] sm:text-xs hidden md:table-cell px-1.5 sm:px-2 py-1 sm:py-1.5">
                        {t("reports_col_catalog")}
                      </TableHead>
                      <TableHead className="text-right text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                        {t("reports_col_unit_price_cost")}
                      </TableHead>
                      <TableHead className="text-right text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                        {t("reports_col_sale_price")}
                      </TableHead>
                      <TableHead className="text-right text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                        {t("reports_col_profit_per_unit")}
                      </TableHead>
                      <TableHead className="text-right text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                        {t("reports_col_profit_margin_percent")}
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {productProfitabilityData
                      ...
                      .map((p) => (
                        <TableRow key={p.id}>
                          <TableCell className="font-medium text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5 truncate max-w-[100px] sm:max-w-xs">
                            {p.name}
                          </TableCell>
                          <TableCell className="text-[10px] sm:text-xs hidden md:table-cell px-1.5 sm:px-2 py-1 sm:py-1.5">
                            {p.catalogNumber}
                          </TableCell>
                          <TableCell className="text-right text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                            {formatNumber(p.unitPrice, t, {
                              currency: true,
                              decimals: 0,
                            })}
                          </TableCell>
                          ...
                          <TableCell className="text-right text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                            {p.potentialProfitPerUnit !== undefined
                              ? formatNumber(p.potentialProfitPerUnit, t, {
                                  currency: true,
                                  decimals: 0,
                                })
                              : "-"}
                          ...
                          <TableCell className="text-right text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                            {p.profitMarginPercent !== undefined
                              ? `${formatNumber(p.profitMarginPercent, t, {
                                  decimals: 1,
                                })}%`
                              : "-"}
                          </TableCell>
              ...
              <p className="text-center text-muted-foreground py-8 sm:py-10 text-xs sm:text-sm">
                {t("reports_profitability_no_data")}
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <div className="grid gap-4 md:grid-cols-1 lg:grid-cols-2">
          <Card
            className="w-full overflow-hidden scale-fade-in"
            style={{ animationDelay: "0.1s" }}
          >
            <CardHeader className="pb-2 sm:pb-4">
              <CardTitle className="text-base sm:text-lg">
                {t("reports_chart_value_over_time_title")}
              </CardTitle>
              <CardDescription>
                {t("reports_chart_value_over_time_desc")}
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0 sm:p-0">
              {valueOverTime.length > 0 ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <RechartsXAxis
                        ...
                        interval={
                          isMobile
                            ? Math.max(
                                0,
                                Math.floor(valueOverTime.length / 3) - 1
                              )
                            : "preserveStartEnd"
                        }
                      />
                      <RechartsYAxis
                        ...
                        axisLine={false}
                        tickFormatter={(value) =>
                          `${t("currency_symbol")}${formatNumber(
                            value / 1000,
                            t,
                            { decimals: 0 }
                          )}k`
                      ...
                      <Line
                        ...
                        dot={false}
                        activeDot={{ r: 4 }}
                        name={t(chartConfig.value.labelKey)}
                      />
                    </LineChart>
                  </ResponsiveContainer>
                </ChartContainer>
                ...
                <p className="text-center text-muted-foreground py-8 sm:py-10 text-xs sm:text-sm">
                  {t("reports_chart_no_value_trend_data")}
                </p>
              )}
            </CardContent>
          </Card>
          ...
          >
            <CardHeader className="pb-2 sm:pb-4">
              <CardTitle className="text-base sm:text-lg">
                {t("reports_chart_docs_processed_title")}
              </CardTitle>
              <CardDescription>
                {t("reports_chart_docs_processed_desc")}
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0 sm:p-0">
              {processingVolume.length > 0 ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <RechartsXAxis
                        ...
                        interval={
                          isMobile
                            ? Math.max(
                                0,
                                Math.floor(processingVolume.length / 2) - 1
                              )
                            : "preserveStartEnd"
                        }
                      />
                      ...
                      <Bar
                        ...
                        name={t(chartConfig.documents.labelKey)}
                      />
                    </BarChart>
                  </ResponsiveContainer>
                </ChartContainer>
                ...
                <p className="text-center text-muted-foreground py-8 sm:py-10 text-xs sm:text-sm">
                  {t("reports_chart_no_processing_volume_data")}
                </p>
              )}
            </CardContent>
          </Card>
          ...
          <Card
            className="w-full overflow-hidden scale-fade-in md:col-span-full lg:col-span-2"
            style={{ animationDelay: "0.3s" }}
          >
            <CardHeader className="pb-2 sm:pb-4 flex flex-row items-center justify-between">
              <div className="flex items-center">
                <CardTitle className="text-base sm:text-lg">
                  {t("reports_expenses_by_category_title")}
                </CardTitle>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                  ...
                  <TooltipContent>
                    <p>{t("reports_expenses_by_category_tooltip")}</p>
                  </TooltipContent>
                </Tooltip>
              </div>
              <Button
                ...
                onClick={() =>
                  exportToCsv(
                    ...
                    [
                      { key: "category", label: t("reports_col_category") },
                      {
                        key: "totalAmount",
                        label: t("reports_col_total_amount"),
                      },
                    ],
            ...
            <CardContent className="p-0 sm:p-0">
              {expensesByCategoryData.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-0 md:gap-4 items-center">
                  <div
                    className={cn(
                      "md:col-span-1 max-h-[280px] overflow-y-auto px-2 py-2 md:py-0",
                      isMobile && "w-full"
                    )}
                  >
                    <Table>
                      ...
                      <TableBody>
                        {expensesByCategoryData.map((item) => (
                          <TableRow key={item.category}>
                            <TableCell className="font-medium text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                              {item.category}
                            </TableCell>
                            <TableCell className="text-right text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                              {formatNumber(item.totalAmount, t, {
                                currency: true,
                                decimals: 0,
                              })}
                            </TableCell>
                  ...
                  <div
                    className={cn(
                      "md:col-span-1 flex items-center justify-center p-0 sm:pb-2 h-[180px] sm:h-[220px]",
                      isMobile && expensesByCategoryData.length > 0 && "mt-2",
                      expensesByCategoryData.length === 0 &&
                ...
                <p className="text-center text-muted-foreground py-8 sm:py-10 text-xs sm:text-sm">
                  {t("reports_chart_no_expenses_by_category_data")}
                </p>
              )}
            </CardContent>
          </Card>
          ...
          <Card
            className="md:col-span-full lg:col-span-1 w-full overflow-hidden scale-fade-in"
            style={{ animationDelay: "0.4s" }}
          >
            <CardHeader className="pb-2 sm:pb-4 flex flex-row items-center justify-between">
              <CardTitle className="text-base sm:text-lg">
                {t("reports_table_top_selling_title")}
              </CardTitle>
              <Button
                variant="ghost"
                size="sm"
                onClick={() =>
                  exportToCsv(
                    ...
                    [
                      ...
                      {
                        key: "quantitySold",
                        label: t("reports_table_col_qty_sold"),
                      },
                      {
                        key: "totalValue",
                        label: t("reports_table_col_total_value"),
                      },
                    ],
            ...
            <CardContent className="p-0">
              {topSellingProducts.length > 0 ? (
                <div className="overflow-x-auto">
                  <Table className="min-w-full">
                    <TableHeader>
                      <TableRow>
                        <TableHead className="text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                          {t("reports_table_col_product")}
                        </TableHead>
                        <TableHead className="text-right text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                          {t("reports_table_col_qty_sold")}
                        </TableHead>
                        <TableHead className="text-right text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                          {t("reports_table_col_total_value")}
                        </TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {topSellingProducts.map((product, index) => (
                        <TableRow key={product.id || index}>
                          ...
                          <TableCell className="text-right text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                            {formatNumber(product.quantitySold, t, {
                              decimals: 0,
                            })}
                          </TableCell>
                          <TableCell className="text-right text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                            {formatNumber(product.totalValue, t, {
                              currency: true,
                              decimals: 0,
                            })}
                          </TableCell>
                ...
                <p className="text-center text-muted-foreground py-8 sm:py-10 text-xs sm:text-sm">
                  {t("reports_table_no_top_selling_data")}
                </p>
              )}
            </CardContent>
          </Card>
            ...
            <CardHeader className="pb-2 sm:pb-4 flex flex-col">
              <div className="flex items-center justify-between">
                ...
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p>{t("reports_pnl_summary_tooltip")}</p>
                  </TooltipContent>
                </Tooltip>
              </div>
              <CardDescription className="text-xs text-muted-foreground">
                {t("reports_pnl_summary_desc")}
              </CardDescription>
            </CardHeader>
                ...
                <>
                  <div className="flex justify-between text-sm items-center py-1 border-b">
                    <span className="text-muted-foreground">
                      {t("reports_pnl_income")}
                    </span>
                    <span className="font-semibold">
                      {formatNumber(profitAndLossData.income, t, {
                        currency: true,
                        decimals: 0,
                      })}
                    </span>
                  ...
                  <div className="flex justify-between text-sm items-center py-1 border-b">
                    <span className="text-muted-foreground">
                      {t("reports_pnl_operating_expenses")}
                    </span>
                    <span className="font-semibold">
                      {formatNumber(profitAndLossData.expenses, t, {
                        currency: true,
                        decimals: 0,
                      })}
                    </span>
                  ...
                  <div className="flex justify-between text-sm items-center py-1 border-b">
                    <span className="text-muted-foreground">
                      {t("reports_pnl_open_liabilities")}
                    </span>
                    <span className="font-semibold">
                      {formatNumber(profitAndLossData.liabilities, t, {
                        currency: true,
                        decimals: 0,
                      })}
                    </span>
                  ...
                  <div
                    className={cn(
                      "flex justify-between text-base font-semibold items-center py-1.5",
                      profitAndLossData.net < 0 && "text-destructive"
                    )}
                  >
                    <span>{t("reports_pnl_net_profit_loss")}</span>
                    <span>
                      {formatNumber(profitAndLossData.net, t, {
                        currency: true,
                        decimals: 0,
                      })}
                    </span>
          ...
          >
            <CardHeader className="pb-2 sm:pb-4 flex flex-row items-center justify-between">
              <CardTitle className="text-base sm:text-lg">
                {t("reports_supplier_liabilities_title")}
              </CardTitle>
              <Button
                variant="ghost"
                size="sm"
                onClick={() =>
                  exportToCsv(
                    ...
                    [
                      { key: "supplierName", label: t("reports_col_supplier") },
                      { key: "totalDue", label: t("reports_col_total_due") },
                      {
                        key: "invoiceCount",
                        label: t("reports_col_invoice_count"),
                      },
                    ],
              ...
              {supplierLiabilitiesData.length > 0 ? (
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                        {t("reports_col_supplier")}
                      </TableHead>
                      <TableHead className="text-right text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                        {t("reports_col_total_due")}
                      </TableHead>
                      <TableHead className="text-right text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                        {t("reports_col_invoice_count")}
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                ...
                <p className="text-center text-muted-foreground py-8 sm:py-10 text-xs sm:text-sm">
                  {t("reports_no_supplier_liabilities_data")}
                </p>
              )}
            </CardContent>
          </Card>
          ...
          <Card
            className="md:col-span-full lg:col-span-2 w-full overflow-hidden scale-fade-in"
            style={{ animationDelay: "0.7s" }}
          >
            <CardHeader className="pb-2 sm:pb-4 flex flex-row items-center justify-between">
              <div className="flex items-center">
                <CardTitle className="text-base sm:text-lg">
                  {t("reports_table_stock_alert_title")}
                </CardTitle>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                  ...
                  <TooltipContent>
                    <p>{t("reports_stock_alerts_tooltip")}</p>
                  </TooltipContent>
                </Tooltip>
              </div>
              <Button
                ...
                onClick={() =>
                  exportToCsv(
                    ...
                    [
                      ...
                      {
                        key: "catalogNumber",
                        label: t("reports_table_col_catalog"),
                      },
                      {
                        key: "quantity",
                        label: t("reports_table_col_current_qty"),
                      },
                      {
                        key: "minStock",
                        label: t("reports_table_col_min_stock"),
                      },
                      {
                        key: "maxStock",
                        label: t("reports_table_col_max_stock"),
                      },
            ...
            <CardContent className="p-0 max-h-[320px] overflow-y-auto">
              {stockAlerts.length > 0 ? (
                  ...
                  <Table className="min-w-full">
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="text-[10px] sm:text-xs hidden md:table-cell px-1.5 sm:px-2 py-1 sm:py-1.5">
                          {t("reports_table_col_catalog")}
                        </TableHead>
                        <TableHead className="text-right text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                          {t("reports_table_col_current_qty")}
                        </TableHead>
                        <TableHead className="text-right text-[10px] sm:text-xs hidden sm:table-cell px-1.5 sm:px-2 py-1 sm:py-1.5">
                          {t("reports_table_col_min_stock")}
                        </TableHead>
                        <TableHead className="text-right text-[10px] sm:text-xs hidden sm:table-cell px-1.5 sm:px-2 py-1 sm:py-1.5">
                          {t("reports_table_col_max_stock")}
                        </TableHead>
                        <TableHead className="text-right text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                          {t("reports_table_col_status")}
                        </TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {stockAlerts.map((alert) => (
                        <TableRow key={alert.id}>
                          ...
                          <TableCell className="text-right text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                            {formatNumber(alert.quantity, t, { decimals: 0 })}
                          </TableCell>
                          <TableCell className="text-right text-[10px] sm:text-xs hidden sm:table-cell px-1.5 sm:px-2 py-1 sm:py-1.5">
                            {alert.isDefaultMinStock &&
                            alert.status === "Low Stock"
                              ? `${formatNumber(10, t, { decimals: 0 })} (${t(
                                  "reports_default_min_stock_suffix"
                                )})`
                              : alert.minStock !== undefined &&
                                alert.minStock !== null
                              ? formatNumber(alert.minStock, t, { decimals: 0 })
                              : "-"}
                          </TableCell>
                          <TableCell className="text-right text-[10px] sm:text-xs hidden sm:table-cell px-1.5 sm:px-2 py-1 sm:py-1.5">
                            {alert.maxStock !== undefined &&
                            alert.maxStock !== null
                              ? formatNumber(alert.maxStock, t, { decimals: 0 })
                              : "-"}
                          </TableCell>
                          <TableCell className="text-right text-[10px] sm:text-xs px-1.5 sm:px-2 py-1 sm:py-1.5">
                            <Badge
                              ...
                              className={cn(
                                ...
                                alert.status === "Over Stock" &&
                                  "bg-orange-100 text-orange-800 dark:bg-orange-900 dark:text-orange-200 hover:bg-orange-100/80"
                              )}
                            >
                              <AlertTriangle className="mr-0.5 sm:mr-1 h-2.5 w-2.5 sm:h-3 sm:w-3" />
                              {t(
                                `reports_stock_status_${alert.status
                                  .toLowerCase()
                                  .replace(/ /g, "_")}` as any
                              ) || alert.status}
                            </Badge>
                ...
                <p className="text-center text-muted-foreground py-8 sm:py-10 text-xs sm:text-sm">
                  {t("reports_table_no_stock_alerts")}
                </p>
              )}
            </CardContent>
          </Card>
        ...
        <Dialog
          ...
          onOpenChange={setIsDrillDownDialogOpen}
        >
          <DialogContent className="sm:max-w-lg">
            <DialogHeader>
              <DialogTitle>{drillDownDialogTitle}</DialogTitle>
              <DialogDescription>
                {t("reports_drilldown_desc_expenses_list")}
              </DialogDescription>
            </DialogHeader>
            <ScrollArea className="max-h-[60vh]">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>
                      {t("reports_drilldown_col_description")}
                    </TableHead>
                    <TableHead>{t("reports_drilldown_col_date")}</TableHead>
                    <TableHead className="text-right">
                      {t("reports_drilldown_col_amount")}
                    </TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                    ...
                    <TableRow key={expense.id}>
                      ...
                      <TableCell>
                        {expense.date instanceof Timestamp
                          ...
                          : t("invoices_invalid_date")}
                      </TableCell>
                      <TableCell className="text-right">
                        {formatNumber(expense.amount, t, {
                          currency: true,
                          decimals: 0,
                        })}
                      </TableCell>
