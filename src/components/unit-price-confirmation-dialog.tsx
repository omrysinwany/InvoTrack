...
import type { Product, ProductPriceDiscrepancy } from "@/services/types";
...
const UnitPriceConfirmationDialog: React.FC<
  UnitPriceConfirmationDialogProps
> = ({
  discrepancies,
  onComplete,
  ...
  const handleConfirm = () => {
    const resolvedProducts: Product[] = discrepancies.map((d) => {
      ...
      return {
        ...
        unitPrice:
          decision === "update_new" ? d.newUnitPrice : d.existingUnitPrice,
      };
    });
    ...
    <Sheet open={isOpen} onOpenChange={internalSheetOpenChangeHandler}>
      ...
      >
        ...
        <div className="p-3 sm:p-4 border-b flex flex-col sm:flex-row gap-2">
          <Button
            onClick={handleUpdateAllToNew}
            variant="outline"
            size="sm"
            className="w-full sm:w-auto"
          >
            <ChevronsUpDown className="mr-2 h-4 w-4" />{" "}
            {t("unit_price_confirmation_update_all_button")}
          </Button>
          <Button
          ...
          >
            <ChevronsUpDown className="mr-2 h-4 w-4" />{" "}
            {t("unit_price_confirmation_keep_all_button")}
          </Button>
        </div>
        ...
        <ScrollArea className="flex-grow">
          <div className="space-y-3 p-3 sm:p-4">
            {discrepancies.map((d) => (
              <div
                key={d.id}
                className="p-3 border rounded-md shadow-sm bg-background"
              >
                {" "}
                {/* שינוי ל-bg-background או bg-card */}
                <p className="font-medium text-sm">
                  {d.shortName || d.description}
                </p>
                <p className="text-xs text-muted-foreground">
                  {t("unit_price_confirmation_catalog_label")}:{" "}
                  {d.catalogNumber}
                </p>
                <div className="grid grid-cols-2 gap-2 mt-2 text-xs">
                  <div>
                    <p className="font-semibold">
                      {t("unit_price_confirmation_current_price_label")}:
                    </p>
                    <p>{formatCurrency(d.existingUnitPrice)}</p>
                  </div>
                  <div>
                    <p className="font-semibold text-amber-600 dark:text-amber-400">
                      {t("unit_price_confirmation_new_price_label")}:
                    </p>
                    <p className="text-amber-600 dark:text-amber-400">
                      {formatCurrency(d.newUnitPrice)}
                    </p>
                  </div>
                </div>
                <RadioGroup
                  ...
                  onValueChange={(value) =>
                    handleDecisionChange(d.id, value as PriceDecision)
                  }
                ...
                >
                  <div className="flex items-center space-x-2">
                    ...
                    <Label
                      htmlFor={`keep_old_${d.id}`}
                      className="text-xs font-normal cursor-pointer"
                    >
                      {t("unit_price_confirmation_option_keep_old", {
                        price: formatCurrency(d.existingUnitPrice),
                      })}
                    </Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <Label
                      htmlFor={`update_new_${d.id}`}
                      className="text-xs font-normal cursor-pointer"
                    >
                      {t("unit_price_confirmation_option_update_new", {
                        price: formatCurrency(d.newUnitPrice),
                      })}
                    </Label>
                  </div>
                </RadioGroup>
        ...
        <SheetFooter className="p-3 sm:p-4 border-t flex flex-col sm:flex-row gap-2 shrink-0">
          ...
          <SheetClose asChild>
            ...
            >
              ...
              {t("unit_price_confirmation_cancel_button")}
            </Button>
          </SheetClose>
          <Button
          ...
          >
            ...
            {t("unit_price_confirmation_confirm_button")}
          </Button>
        </SheetFooter>
      </SheetContent>
    </Sheet>
