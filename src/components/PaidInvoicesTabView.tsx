...
import {
  ...
  CreditCard,
  MailIcon,
} from "lucide-react"; // Added MailIcon
...
interface PaidInvoicesTabViewProps {
  ...
  handleViewDetails: (
    ...
    context?: "image_only" | "full_details"
  ) => void;
...
export default function PaidInvoicesTabView({
  ...
  const formatDateForDisplay = useCallback(
    (
      dateInput: string | Date | Timestamp | FieldValue | undefined,
      ...
      try {
        ...
        if (!dateObj || !isValid(dateObj)) {
          console.warn(
            "[PaidInvoicesTabView] Invalid date object or unhandled FieldValue for input:",
            dateInput
          );
  ...
  const fetchPaidInvoices = useCallback(async () => {
    ...
    try {
      ...
      fetchedData = fetchedData.filter(
        (inv) => inv.paymentStatus === "paid" && inv.status === "completed"
      );

      if (filterDocumentType) {
        fetchedData = fetchedData.filter(
          (inv) => inv.documentType === filterDocumentType
        );
      }
      if (filterSupplier) {
        fetchedData = fetchedData.filter(
          (inv) => inv.supplierName === filterSupplier
        );
      }
      if (dateRange?.from) {
  ...
  const displayedPaidInvoices = useMemo(() => {
    ...
    return paidInvoices.slice(
      startIndex,
      startIndex + ITEMS_PER_PAGE_PAID_INVOICES
    );
  ...
  const handlePaymentReceiptUploaded = async (
    ...
    if (
      ...
      !user?.id ||
      invoiceForReceiptUpload.id !== invoiceId
    )
      return;
    ...
    try {
      ...
      toast({
        ...
        description: t("paid_invoices_toast_receipt_uploaded_desc", {
          fileName:
            paidInvoices.find((inv) => inv.id === invoiceId)
  ...
  return (
    <>
      {viewMode === "list" ? (
          ...
          <Table className="min-w-[600px]">
                ...
                {visibleColumnHeadersPaid.map((header) => (
                  <TableHead
                    ...
                    className={cn(
                      ...
                      "px-2 sm:px-4 py-2",
                      header.key === "actions"
                        ? "sticky left-[calc(var(--checkbox-width,3%)+0.25rem)] bg-card z-10"
                        : header.key === "selection"
                        ? "sticky left-0 bg-card z-20"
                        : ""
                    )}
                    onClick={() =>
                      header.sortable &&
                      parentHandleSort(header.key as SortKeyPaid)
                    }
                    aria-sort={
                      header.sortable
                      ...
                      {header.key === "selection" ? (
                        <Checkbox
                          checked={
                            ...
                            selectedInvoiceIds.length === paidInvoices.length &&
                            paidInvoices.length > 0
                          }
                          onCheckedChange={(checked) =>
                            handleSelectInvoice("all-paid", !!checked)
                          }
            ...
            <TableBody>
              {isLoading ? (
                <TableRow>
                  <TableCell
                    colSpan={visibleColumnHeadersPaid.length}
                    className="h-24 text-center px-2 sm:px-4 py-2"
                  >
                ...
                displayedPaidInvoices.map((item) => (
                  <TableRow
                    ...
                    data-testid={`paid-invoice-item-${item.id}`}
                  >
                    {visibleColumns.selection && (
                      <TableCell
                        className={cn(
                          "text-center px-1 sm:px-2 py-2 sticky left-0 bg-card z-20",
                          columnDefinitions.find((h) => h.key === "selection")
                            ?.className
                        )}
                      >
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            handleSelectInvoice(item.id, !!checked)
                          }
                          aria-label={t("invoice_export_select_aria", {
                            fileName:
                    ...
                    {visibleColumns.actions && (
                      <TableCell
                        className={cn(
                          "text-center px-1 sm:px-2 py-2 sticky left-[calc(var(--checkbox-width,3%)+0.25rem)] bg-card z-10",
                          columnDefinitions.find((h) => h.key === "actions")
                            ?.className
                        )}
                      >
                        <Button
                          ...
                          onClick={() =>
                            handleViewDetails(item, "full_details")
                          }
                          title={t("invoices_view_details_title", {
                            fileName:
                              item.originalFileName ||
                              item.generatedFileName ||
                              "",
                        ...
                        <Button
                          ...
                          onClick={() => {
                            setInvoiceForReceiptUpload(item);
                    ...
                    {visibleColumns.originalFileName && (
                      <TableCell
                        className={cn(
                          "font-medium px-2 sm:px-4 py-2",
                          columnDefinitions.find(
                            (h) => h.key === "originalFileName"
                          )?.className
                        )}
                      >
                        <Button
                    ...
                    {visibleColumns.uploadTime && (
                      <TableCell
                        className={cn(
                          ...
                          columnDefinitions.find((h) => h.key === "uploadTime")
                    ...
                    {visibleColumns.supplierName && (
                      <TableCell
                        className={cn(
                          "px-2 sm:px-4 py-2 text-center",
                          columnDefinitions.find(
                            (h) => h.key === "supplierName"
                          )?.mobileHidden && "hidden sm:table-cell"
                        )}
                      >
                        {item.supplierName || t("invoices_na")}
                      </TableCell>
                    )}
                    {visibleColumns.totalAmount && (
                      <TableCell className="text-right px-2 sm:px-4 py-2 whitespace-nowrap">
                        ...
                        item.totalAmount !== null
                          ? formatCurrencyDisplay(item.totalAmount, {
                              decimals: 0,
                            })
                          : t("invoices_na")}
                      </TableCell>
                    ...
                    {visibleColumns.paymentMethod && (
                      <TableCell
                        className={cn(
                          "px-2 sm:px-4 py-2 text-center",
                          columnDefinitions.find(
                            (h) => h.key === "paymentMethod"
                          )?.mobileHidden && "hidden sm:table-cell"
                        )}
                      >
                        {item.paymentMethod
                          ? t(
                              `payment_method_${item.paymentMethod
                                .toLowerCase()
                    ...
                    {visibleColumns.paymentReceiptImageUri && (
                      <TableCell
                        className={cn(
                          "px-2 sm:px-4 py-2 text-center",
                          columnDefinitions.find(
                            (h) => h.key === "paymentReceiptImageUri"
                          )?.mobileHidden && "hidden sm:table-cell"
                        )}
                      >
                        {item.paymentReceiptImageUri ? (
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              handleViewDetails(item, "image_only")
                            }
                            className="p-0 h-auto text-xs"
                          >
                            {t("paid_invoices_view_receipt_link")}
                          </Button>
                        ) : (
                          t("invoices_na")
                        )}
            ...
            <div className="flex items-center justify-end space-x-2 py-4">
              ...
              <span className="text-sm text-muted-foreground">
                {t("inventory_pagination_page_info_simple", {
                  ...
                  totalPages: totalPaidInvoicesPages,
                })}
              </span>
              <Button
                variant="outline"
        ...
        >
          {isLoading ? (
            Array.from({ length: ITEMS_PER_PAGE_PAID_INVOICES }).map(
              (_, index) => (
            ...
            displayedPaidInvoices.map((item) => (
              <Card
                key={item.id}
                className="flex flex-col overflow-hidden cursor-pointer hover:shadow-lg transition-shadow scale-fade-in"
              >
                ...
                <CardHeader
                  className="p-0 relative aspect-[4/3]"
                  onClick={() => handleViewDetails(item, "image_only")}
                >
                  {isValidImageSrc(
                    item.paymentReceiptImageUri || item.originalImagePreviewUri
                  ) ? (
                    <NextImage
                      src={
                        item.paymentReceiptImageUri ||
                      ...
                      alt={t("paid_invoices_receipt_image_alt", {
                        fileName:
                          item.originalFileName || item.generatedFileName || "",
                      })}
                      layout="fill"
                  ...
                  <div className="absolute top-2 right-2 flex flex-col gap-1">
                    <Badge
                      variant="secondary"
                      className="bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200 hover:bg-green-100/80 text-[10px] sm:text-xs font-medium px-1.5 sm:px-2 py-0.5"
                    >
                      <CreditCard className="mr-1 h-3 w-3" />
                      {t("invoice_payment_status_paid")}
                    </Badge>
                  </div>
                </CardHeader>
                <CardContent
                ...
                >
                  ...
                  {item.supplierName && (
                    <p className="text-xs text-muted-foreground">
                      {t("invoice_details_supplier_label")}: {item.supplierName}
                    </p>
                  )}
                  {item.totalAmount !== undefined && (
                    <p className="text-xs font-medium">
                      {t("invoices_col_total")}:{" "}
                      {formatCurrencyDisplay(item.totalAmount, { decimals: 0 })}
                    </p>
                  )}
                </CardContent>
                <CardFooter className="p-3 border-t flex gap-1">
                  ...
                  >
                    ...
                    {t("invoices_view_details_button")}
                  </Button>
                  <Button
                    variant="ghost"
                    size="sm"
                    className="flex-1 justify-start text-xs text-blue-600 hover:text-blue-500"
                    onClick={(e) => {
                      e.stopPropagation();
                      setInvoiceForReceiptUpload(item);
                      setShowReceiptUploadDialog(true);
                  ...
                  >
                    ...
                    {t("paid_invoices_update_receipt_button")}
                  </Button>
                </CardFooter>
              </Card>
            ))
            ...
            <AlertDialogContentComponent>
              <AlertDialogHeaderComponent>
                <AlertDialogTitleComponent>
                  {t("invoices_delete_confirm_title")}
                </AlertDialogTitleComponent>
                <AlertDialogDescriptionComponent>
                  {t("invoices_delete_confirm_desc", {
                    fileName: `${selectedInvoiceIds.length} ${t(
                      "documents_plural_for_delete_message"
                    )}`,
                  })}
                </AlertDialogDescriptionComponent>
              </AlertDialogHeaderComponent>
                ...
                <AlertDialogAction
                  onClick={async () => {
                    ...
                    try {
                      ...
                      toast({
                        title: t("invoices_toast_bulk_deleted_title"),
                        description: t("invoices_toast_bulk_deleted_desc", {
                          count: selectedInvoiceIds.length,
                        }),
                      });
                      fetchPaidInvoices();
      ...
      {showReceiptUploadDialog && invoiceForReceiptUpload && (
        <PaymentReceiptUploadDialog
          ...
          onConfirmUpload={async (receiptUri) => {
            await handlePaymentReceiptUploaded(
              invoiceForReceiptUpload.id,
              receiptUri
            );
          }}
        />
