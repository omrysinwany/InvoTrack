...
import type { Supplier, DueDateOption } from "@/services/types";
...
const SupplierPaymentSheet: React.FC<SupplierPaymentSheetProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      console.log(
        "[SupplierPaymentSheet] Opening/Resetting State. Scanned Name:",
        potentialSupplierNameFromScan,
        "Scanned Osek Morshe:",
        potentialOsekMorsheFromScan
      );
  ...
  const handleSelectedSupplierChange = (supplierId: string) => {
    ...
    if (supplier) {
      ...
      // If for some reason the selected supplier is not found, revert to the scanned value as a fallback
      setOsekMorsheInput(potentialOsekMorsheFromScan || "");
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (supplierOption === "use_new") {
      ...
      finalOsekMorshe =
        chosenSupplier.osekMorshe || osekMorsheInput.trim() || null;
      isNew = false;
    }
